...
const Products = () => {
  const api = 'http://localhost:5200/products';
  ...
  const filteredProducts = products.filter((product) => {
    const name = (product && product.productname) ? String(product.productname).toLowerCase() : '';
  ...
  const categories = [
    ...new Set(
      products
        .map((product) => (product && product.category ? String(product.category).toLowerCase() : ''))
        .filter(Boolean)
    ),
  ];
