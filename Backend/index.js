...
// require DB connection (connect.js will use process.env.MONGO_URI)
require('./db/connect');
...
app.post('/adminlogin', async (req, res) => {
    ...
    const token = jwt.sign({ userId: user._id, role: isAdmin ? 'admin' : 'admin' }, JWT_SECRET, { expiresIn: '7d' });
    res.json({ user, token });
});
...
app.post('/login', async (req, res) => {
    ...
    // Generate a JWT token
    const token = jwt.sign({ userId: user._id, role: isAdmin ? 'admin' : 'user' }, JWT_SECRET, { expiresIn: '7d' });
    res.json({ user, token });
});
